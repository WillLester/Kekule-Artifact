...
''')
    ...
    def generate_target(self, target):
        ...
        if self.environment.is_build_llvm_ir():
            if not outname.endswith('.ll'):
                outname = outname + '.ll'
    ...
    @classmethod
    def compiler_to_llvm_rule_name(cls, compiler: Compiler) -> str:
        return cls.get_compiler_rule_name("{}_llvm_ir".format(compiler.get_language()), compiler.for_machine)
...
https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47485'''))
    ...
    def generate_compile_rule_for(self, langname, compiler):
        ...
        if self.environment.is_build_llvm_ir():
            rule = self.compiler_to_llvm_rule_name(compiler)
        else:
            rule = self.compiler_to_rule_name(compiler)
        if langname == 'cuda':
        ...
        command = compiler.get_exelist()
        if langname.endswith("_llvm_ir"):
            args = ['$ARGS'] + depargs + ['-S', '-emit-llvm', '-fno-inline'] + NinjaCommandArg.list(compiler.get_output_args('$out'), Quoting.none) + \
                    compiler.get_compile_only_args() + ['$in']
        else:
            args = ['$ARGS'] + depargs + NinjaCommandArg.list(compiler.get_output_args('$out'), Quoting.none) + compiler.get_compile_only_args() + \
                   ['$in']
    ...
    def generate_compile_rule_for_llvm_ir(self, langname, compiler):
        if langname != 'c' and langname != 'cpp':
            return
        if compiler.get_id() != 'clang':
            return
        return self.generate_compile_rule_for('{}_llvm_ir'.format(langname), compiler)
    ...
    def generate_compile_rules(self):
        for for_machine in MachineChoice:
            ...
            for langname, compiler in clist.items():
                ...
                if compiler.get_id() == 'clang' and (langname == 'c'or langname == 'cpp') and self.environment.is_build_llvm_ir():
                    self.generate_compile_rule_for_llvm_ir(langname, compiler)
                else:
                    self.generate_compile_rule_for(langname, compiler)
                self.generate_pch_rule_for(langname, compiler)
                for mode in compiler.get_modes():
                    if compiler.get_id() == 'clang' and (langname == 'c'or langname == 'cpp') and self.environment.is_build_llvm_ir():
                        self.generate_compile_rule_for_llvm_ir(langname, mode)
                    else:
    ...
    def generate_single_compile(self, target: build.BuildTarget, src,
        ...
        if self.environment.is_build_llvm_ir():
            compiler_name = self.compiler_to_llvm_rule_name(compiler)
        else:
    ...
    def generate_link(self, target: build.BuildTarget, outname, obj_list, linker: T.Union['Compiler', 'StaticLinker'], extra_args=None, stdlib_args=None):
        ...
        if not self.environment.is_build_llvm_ir() and isinstance(target, build.SharedLibrary):
            self.generate_shsym(target)
        crstr = self.get_rule_suffix(target.for_machine)
