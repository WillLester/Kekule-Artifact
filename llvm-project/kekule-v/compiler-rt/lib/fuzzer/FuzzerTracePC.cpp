...
TracePC TPC;
RegQueue<AccessVar> LocalReg;
ValueBitMap GlobalReg;
Graph<RegVar> Dependencies;
...
void TracePC::PrintModuleInfo() {
  ...
  size_t MaxFeatures = CollectFeatures([](uint32_t) {}, false);
...
ATTRIBUTE_NO_SANITIZE_ALL
ATTRIBUTE_TARGET_POPCNT
void __sanitizer_reg_trace_push(uint16_t Id, uint8_t Type, bool IsRead, bool IsMarked) {
  fuzzer::LocalReg.Push(fuzzer::AccessVar(Id, Type, IsRead, IsMarked));
}
