...
size_t MutationDispatcher::Mutate_CustomCrossOver(uint8_t *Data, size_t Size,
  ...
  if (Size == 0)
    return 0;
  if (CrossOverWith.empty()) return 0;
  size_t NewSize = 0;
  size_t CrossOverCnt = 0;
  for (size_t i = 0; i < CrossOverWith.size(); ++i) {
    const Unit &Other = *(CrossOverWith[i]);
    if (Other.empty())
      continue;
    ...
    NewSize = EF->LLVMFuzzerCustomCrossOver(
...
size_t MutationDispatcher::MutateImpl(uint8_t *Data, size_t Size,
  ...
  for (int Iter = 0; Iter < 100; Iter++) {
    // Increased crossover probability
