...
public:
  ...
  size_t NumInputsThatTouchFocusFunction() {
    size_t Res = 0;
    for (int i = 0; i < NUM_PRIORITY; ++i)
      Res += std::count_if(Inputs[i].begin(), Inputs[i].end(), [](const InputInfo *II) {
  ...
  const Unit &operator[] (size_t Idx) const {
    return InputInfoAtIdx(Idx)->U;
  }

  InputInfo *AddToCorpus(const Unit &U, size_t NumFeatures, bool MayDeleteFile,
                         bool HasFocusFunction, bool NeverReduce, uint8_t Priority,
                         std::chrono::microseconds TimeOfUnit,
                         const std::vector<uint32_t> &FeatureSet,
                         const DataFlowTrace &DFT, const InputInfo *BaseII) {
    assert(!U.empty());
    if (FeatureDebug)
      Printf("ADD_TO_CORPUS %zd NF %zd\n", InputsSize(), NumFeatures);
    // InputsSize() is cast to uint32_t below.
    uint8_t Idx = Priority;
    assert(Inputs[Idx].size() < std::numeric_limits<uint32_t>::max());
    Inputs[Idx].push_back(new InputInfo());
    InputInfo &II = *Inputs[Idx].back();
    II.U = U;
  ...
  InputInfo &ChooseUnitToMutate(Random &Rand) {
    InputInfo &II = *InputInfoAtIdx(ChooseUnitIdxToMutate(Rand));
    assert(!II.U.empty());
    return II;
  }
  ...
  InputInfo &ChooseUnitToCrossOverWith(Random &Rand, bool UniformDist) {
    ...
    InputInfo &II = *InputInfoAtIdx(Rand(InputsSize()));
    assert(!II.U.empty());
    return II;
  }
  ...
  void PrintFeatureSet() {
    ...
    for (int i = 0; i < NUM_PRIORITY; ++i)
      for (size_t j = 0; j < Inputs[i].size(); j++)
        if (size_t N = Inputs[i][j]->NumFeatures)
          Printf(" %zd=>%zd ", i, N);
    Printf("\n");
  }
  ...
  void DeleteInput(size_t Idx) {
    InputInfo &II = *InputInfoAtIdx(Idx);
    DeleteFile(II);
    Unit().swap(II.U);
    II.Energy = 0.0;
    II.NeedsEnergyUpdate = false;
  ...
  bool AddFeature(size_t Idx, uint32_t NewSize, bool Shrink) {
    ...
    if (OldSize == 0 || (Shrink && OldSize > NewSize)) {
      if (OldSize > 0) {
        ...
        InputInfo &II = *InputInfoAtIdx(OldIdx);
      ...
      // InputsSize() is guaranteed to be less than UINT32_MAX by AddToCorpus.
      SmallestElementPerFeature[Idx] = static_cast<uint32_t>(InputsSize());
...
private:
  ...
  InputInfo* InputInfoAtIdx(size_t Idx) const {
    for (int i = 0; i < NUM_PRIORITY; ++i) {
      if (Idx >= Inputs[i].size()) {
        Idx -= Inputs[i].size();
      } else {
        return Inputs[i][Idx];
      }
    }
    return Inputs[NUM_PRIORITY - 1][Idx];
  }
  ...
  void ValidateFeatureSet() {
    ...
    for (size_t Idx = 0; Idx < kFeatureSetSize; Idx++)
      if (GetFeature(Idx))
        InputInfoAtIdx(SmallestElementPerFeature[Idx])->Tmp++;
    for (int i = 0; i < NUM_PRIORITY; ++i) {
      for (auto II: Inputs[i]) {
  ...
  void UpdateCorpusDistribution(Random &Rand) {
    ...
    if (Entropic.Enabled) {
      ...
      for (size_t i = 0; i < N; ++i) {
        InputInfo* Info = InputInfoAtIdx(i);
        if (Info->NumFeatures == 0 && Info->Priority <= 1) {
          // If the seed doesn't represent any features and it has no priority, assign zero energy.
          Weights[i] = 0.;
        } else if (Info->NumExecutedMutations / kMaxMutationFactor >
                   NumExecutedMutations / InputsSize()) {
          // If the seed was fuzzed a lot more than average, assign zero energy.
          Weights[i] = 0.;
        } else {
          // Otherwise, simply assign the computed energy.
          Weights[i] = Info->Energy;
        }
          ...
          Weights[i] =
              Info->NumFeatures
                  ? static_cast<double>((i + 1) *
                                        (Info->HasFocusFunction ? 1000 : 1))
                  : 0.;
        } else {
          Weights[i] = Info->Priority * static_cast<double>((i + 1) * (Info->HasFocusFunction ? 1000 : 1));
        }
      }
    }
    ...
    if (FeatureDebug) {
      for (size_t i = 0; i < N; i++)
        Printf("%zd ", InputInfoAtIdx(i)->NumFeatures);
      Printf("SCORE\n");
      for (size_t i = 0; i < N; i++)
        Printf("%f ", Weights[i]);
      Printf("Weights\n");
  ...
  std::vector<InputInfo *> Inputs[NUM_PRIORITY];
