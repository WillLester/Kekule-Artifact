...
static Fuzzer *F;
static unsigned long DepSum = 0;
static unsigned long DepCov = 0;
static unsigned long long CurSec = 1;
...
void Fuzzer::InitializeDependencies() {
  const char* dependEnvName = "DEPENDENCY_FUZZ";
  const char* dependEnvVal = std::getenv(dependEnvName);
  if (dependEnvVal != NULL) {
    std::ifstream dependFile(dependEnvVal);
	if (dependFile.is_open()) {
	  bool isOddLine = true;
      std::string line;
	  RegVar cur;
	  while (std::getline(dependFile, line)) {
        std::istringstream iss(line);
		std::string token;
        if (isOddLine) {
		  std::getline(iss, token, '.');
		  uint8_t regId = static_cast<uint8_t>(std::stoi(token));
		  std::getline(iss, token, '.');
		  ...
		  while (std::getline(iss, token, ';')) {
            std::istringstream issVar(token);
		    std::getline(issVar, token, '.');
		    uint8_t regId = static_cast<uint8_t>(std::stoi(token));
		    std::getline(issVar, token, '.');
		    uint8_t varId = static_cast<uint8_t>(std::stoi(token));
		    std::getline(issVar, token, '.');
		    bool isRead = static_cast<bool>(std::stoi(token));
			RegVar dep(regId, varId, isRead);
			Dependencies.addEdge(cur, dep);
			DepSum++;
		  }
		}
		isOddLine = !isOddLine;
	...
	Printf("Dependency not enabled\n");
    DependencyEnabled = false;
  }
}

size_t Fuzzer::HashNode(uint8_t Node) {
	return HashValue(Node);
}

size_t Fuzzer::HashEdge(uint8_t Node1, uint8_t Node2, uint8_t CoveredNum) {
	size_t hashValue = HashValue(Node1);
	hashValue = HashCombine(hashValue, Node2);
	hashValue = HashCombine(hashValue, CoveredNum);
	return hashValue;
}

size_t Fuzzer::HashValue(uint8_t Value) {	
	const size_t prime = 31;
	return static_cast<size_t>(Value) * prime;
}

size_t Fuzzer::HashCombine(uint8_t Value1, uint8_t Value2) {
	return Value1 ^ (HashValue(Value2) + 0x9e3779b9 + (Value1 << 6) + (Value1 >> 2));
}
...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size, bool MayDeleteFile,
  ...
  if (DependencyEnabled) {
	...
	while (!LocalReg.IsEmpty()) {
		...
		if (Reg.typeId == 1) {
			...
			if (curBehavior != -1) {
				Seq.push_back(RegVar(curBehavior, Reg.id, Reg.isRead, Reg.isMarked));
			}
		}
		fuzzer::LocalReg.Pop();
	}
  ...
  if (NumNewFeatures || ForceAddToCorpus || FoundNewRegOp) {
    TPC.UpdateObservedPCs();
    auto NewII =
        Corpus.AddToCorpus({Data, Data + Size}, NumNewFeatures, MayDeleteFile,
                           TPC.ObservedFocusFunction(), ForceAddToCorpus, Priority,
...
void Fuzzer::MutateAndTestOne() {
  ...
  if (Options.DoCrossOver) {
    // Randomly choose N units to cross over instead.
    Random& Rand = MD.GetRand();
    size_t N = Rand(CROSS_OVER_LIMIT) + 1;
    MD.CleanCrossOverWith();
    for (size_t i = 0; i < N; ++i) {
      auto &CrossOverII = Corpus.ChooseUnitToCrossOverWith(
        Rand, Options.CrossOverUniformDist);
      MD.AddCrossOverWith(&CrossOverII.U);
    }
  }
  const auto &U = II.U;
...
void Fuzzer::ReadAndExecuteSeedCorpora(std::vector<SizedFile> &CorporaFiles) {
  ...
  if (Corpus.empty() && Options.MaxNumberOfRuns) {
    ...
    Corpus.AddToCorpus({'\n'}, /*NumFeatures=*/1, /*MayDeleteFile=*/true,
                       /*HasFocusFunction=*/false, /*NeverReduce=*/false, /*Priority*/1,
